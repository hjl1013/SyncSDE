...
class IFPieplineEditStage_1(IFPipeline):
    ...
    def __call__(
        ...
        do_ccw: Optional[bool] = None,
        double_rotate: Optional[bool] = None,
    ):
        """
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                rot_factor = 1 if do_ccw else -1
                if double_rotate: rot_factor *= 2
                rotated_latent_1 = torch.from_numpy(
                    np.array([np.rot90(np.array(old_latents[0, j].detach().cpu()), k=rot_factor) for j in range(old_latents.shape[1])])
                ).to(device)
                # print(intermediate_images.shape) # [B, 3, 64, 64]
                intermediate_images[1:2] = intermediate_images[1:2] - gamma_t * inv_lambda * (t.item() / T) * (old_latents[1:2] - rotated_latent_1)
