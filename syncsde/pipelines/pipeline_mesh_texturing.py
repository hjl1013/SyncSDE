...
from syncsde.synctweedies.utils.mesh_utils import *
from syncsde.synctweedies.utils.image_utils import *
from syncsde.synctweedies.renderer.mesh.mesh_renderer import UVProjection as UVP
from syncsde.synctweedies.model.base_model import BaseModel
...
class MeshTexturePipeline(BaseModel):
    ...
    def initialize(self):
        ...
        self.mesh_path = os.path.abspath(self.config.mesh_path)
    ...
    def forward_mapping(
        ...
        **kwargs,
        ...
        if z_t.dim() == 4:
            z_t = z_t.squeeze(0)
        self.uvp.set_texture_map(z_t) # z_t is texture domain
        avg_x_t = self.uvp.render_textured_views()

        foregrounds = [view[:-1] for view in avg_x_t]
        bg_masks = [view[-1:] for view in avg_x_t]
        t = self.model.scheduler.timesteps[cur_index]


        mask = ((foregrounds[view_idx][0] != 0) & (foregrounds[view_idx][0] != 1)).to(torch.uint8)
    ...
    def inverse_mapping(
        ...
        **kwargs
        ...
        rendered_views, z_t, visibility_weights = self.uvp.bake_texture_wo_voronoi(views=views, 
    ...
    def one_step_process(
        ...
        t = timestep
        prev_t = t - scheduler.config.num_train_timesteps // 30
        alpha_cur = scheduler.alphas_cumprod[t]
        alpha_prev = scheduler.alphas_cumprod[prev_t] if prev_t >= 0 else self.scheduler.final_alpha_cumprod
        ...
        for i in range(1, 10):
            z_t_1_0 = self.inverse_mapping(old_xts[0:i], var_type="latent", **kwargs)
            lats, mask = self.forward_mapping(z_t_1_0, i, bg=None, **kwargs)

            x_t_1[i:i+1] = x_t_1[i:i+1] - self.config.inv_lambda * gamma_t * (t / scheduler.config.num_train_timesteps) * mask * (old_xts[i:i+1] - lats[i:i+1])
    ...
    def __call__(self, prompt, seed):
        ...
        prompt = f"Best quality, extremely detailed {prompt}"
        ...
        # SDEdit 
        if self.config.sdedit:
            mesh_root = "/".join(mesh_path.split("/")[:-1])
